...
class FoodMasterList {
  ...
  static final List<FoodModel> foods = [
    ...
    FoodModel(
      ...
      nameLowercase: 'huevos revueltos con aguacate',
      category: 'Ruptura 🍳',
      searchTags: [
        'ruptura suave',
        'grasas saludables',
    ...
    FoodModel(
      ...
      nameLowercase: 'trucha arcoíris (local)',
      category: 'Proteína 🐟',
    ...
    FoodModel(
      ...
      nameLowercase: 'atún enlatado en agua',
      category: 'Proteína 🐟',
    ...
    FoodModel(
      ...
      nameLowercase: 'bacalao (fresco o congelado)',
      category: 'Proteína 🐟',
    ...
    FoodModel(
      ...
      nameLowercase: 'chontaduro (local)',
      category: 'Grasas 🥑',
    ...
    FoodModel(
      ...
      nameLowercase: 'aceite de coco mct',
      category: 'Grasas 🥥',
    ...
    FoodModel(
      ...
      nameLowercase: 'papa pastusa (ensalada fría)',
      category: 'Carbohidratos 🥔',
    ...
    FoodModel(
      ...
      nameLowercase: 'arepa de maíz (tela)',
      category: 'Carbohidratos 🌽',
    ...
    FoodModel(
      ...
      nameLowercase: 'empanada / pan tajado / galletas',
    ...
    FoodModel(
      ...
      nameLowercase: 'yogur con sabor (comercial)',
      category: '⚠️ Inflamatorio',
    ...
    FoodModel(
      ...
      nameLowercase: 'refrescos azucarados',
      category: '⚠️ Inflamatorio',
    ...
    FoodModel(
      ...
      nameLowercase: 'cereales azucarados',
      category: '⚠️ Inflamatorio',
    ...
    FoodModel(
      ...
      nameLowercase: 'cúrcuma + pimienta negra',
      category: 'Especias 🌿',
    ...
    FoodModel(
      ...
      nameLowercase: 'chocolate negro 85%+',
    ...
    FoodModel(
      ...
      nameLowercase: 'kombucha casera (sin azúcar)',
      category: 'Fermentados 🍶',
    ...
    /// Caldo de Huesos — Colágeno hidrolizado + aminoácidos
    FoodModel(
      id: 'caldo-huesos',
      name: 'Caldo de Huesos',
      nameLowercase: 'caldo de huesos',
      category: 'Proteína 🍲',
      searchTags: ['caldo', 'huesos', 'colágeno', 'glicina', 'permeabilidad'],
      protein: 6.0,
      fat: 3.0,
      netCarbs: 0.0,
      calories: 55.0,
      serving: 100.0,
      imrScore: 9,
      tip: 'Colágeno puro (6-8g/taza) + glicina neuromoduladora. '
          'Casero: 12-24h cocción lenta. Gel = signo de colágeno real. Sella intestino permeable.',
      impact: 'sarcopenia',
      level: 2,
      createdAt: DateTime.now(),
      updatedAt: DateTime.now(),
    ...
    FoodModel(
      ...
      nameLowercase: 'levadura nutricional',
  ...
  /// Static method para cargar el dataset en Firestore
  /// Usa batch writes para eficiencia (max 500 ops/batch)
  static Future<void> seedMasterDatabase() async {
    print('[SEED] 🌱 MASTER FOOD DATABASE SEEDING INITIATED');
    print('[SEED] Total foods in dataset: ${foods.length}');

    final firestore = FirebaseFirestore.instance;
    const masterCollection = 'master_food_db';

    try {
      // Check if collection already has documents (avoid re-seeding)
      final existingCount =
          await firestore.collection(masterCollection).count().get();
      final docCount = existingCount.count;

      if (docCount != null && docCount > 0) {
        print(
            '[SEED] ✅ master_food_db already seeded: $docCount documents found');
        print(
            '[SEED] Skipping re-seed to preserve timestamps and manual edits');
        return;
      }

      print('[SEED] 📊 master_food_db is empty. Starting batch write...');

      // Batch writes (FirebaseFirestore max: 500 operations per batch)
      final List<Future<void>> batchFutures = [];
      final int batchSize = 100;

      for (int i = 0; i < foods.length; i += batchSize) {
        final endIndex =
            (i + batchSize < foods.length) ? i + batchSize : foods.length;
        final batchSlice = foods.sublist(i, endIndex);

        final batchFuture =
            _writeBatch(firestore, masterCollection, batchSlice);
        batchFutures.add(batchFuture);
      }

      // Wait all batches to complete
      await Future.wait(batchFutures);

      print(
          '[SEED] ✅ MASTER DATABASE SEEDING COMPLETE: ${foods.length} foods persisted');
      print(
          '[SEED] Collection: master_food_db (indexed for multi-category search)');
    } catch (e) {
      print('[SEED] ❌ Error seeding master database: $e');
      rethrow;
    }
  }

  /// Helper: Write batch of foods to Firestore
  static Future<void> _writeBatch(
    FirebaseFirestore firestore,
    String collection,
    List<FoodModel> batchFoods,
  ) async {
    final batch = firestore.batch();

    for (final food in batchFoods) {
      final docRef = firestore.collection(collection).doc(food.id);
      batch.set(docRef, food.toFirestoreMap());
      print('[SEED] 📝 Queued: ${food.name} (IMR: ${food.imrScore})');
    }

    await batch.commit();
    print('[SEED] ✅ Batch committed: ${batchFoods.length} documents');
  }
}
