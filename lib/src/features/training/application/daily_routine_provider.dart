...
class DailyRoutine extends _$DailyRoutine {
  ...
  Future<List<InteractiveExercise>> build() async {
    ...
    if (log != null && log.completedExercises.isNotEmpty) {
      debugPrint(
          "ElenaApp Log: Log encontrado (Reactive) para $selectedDate. Cargando historial.");
      return log.completedExercises.map((e) {
        ...
        return InteractiveExercise(
          ...
          targetRir:
              "0", // Not stored in log item usually, preserving structure
          sets: setsList
              .map<InteractiveSet>((s) => InteractiveSet(
    ...
    debugPrint(
        "ElenaApp Log: Cargando rutina PLANIFICADA para FECHA: $selectedDate (Dia: $dayIndex)");
    ...
    final dailyWorkout = weeklyPlan.firstWhere(
      ...
      orElse: () {
        debugPrint(
            "ElenaApp Log: No se encontró workout para el día $dayIndex. Retornando Rest.");
        return DailyWorkout(
          dayIndex: dayIndex,
    ...
    if (dailyWorkout.type == WorkoutType.rest) {
      debugPrint(
          "ElenaApp Log: Día de descanso (Day $dayIndex). Retornando lista vacía.");
      return [];
    }
    ...
    if (metabolicState != null) {
      ...
      if (metabolicState.sleepHours < 6.0 ||
          metabolicState.sorenessLevel >= 4) {
        routineType = 'Esencial';
        volumeFactor = 0.7; // Low volume
      } else if (metabolicState.energyLevel >= 8 &&
          metabolicState.nutritionStatus == 'fed') {
      ...
      debugPrint(
          "ElenaApp Logic: Rutina seleccionada: $routineType (Factor: $volumeFactor)");
    }

    // 6. DELOAD LOGIC
    final cycleState = ref.watch(trainingCycleControllerProvider);
    if (cycleState.isDeloadActive) {
      volumeFactor *= 0.5; // Cut volume in half
      debugPrint(
          "ElenaApp Logic: FASE DE DESCARGA ACTIVA. Factor reducido a $volumeFactor");
    }
    ...
    if (dailyWorkout.exercises.isNotEmpty) {
      ...
      return dailyWorkout.exercises.map((e) {
        ...
        if (e.targetMuscle.toLowerCase().contains('tríceps') ||
            e.name.toLowerCase().contains('tríceps')) {
    ...
    debugPrint(
        "ElenaApp Log: Lista de ejercicios vacía para el día $dayIndex.");
    return [];
  }
    ...
    state.whenData((routine) {
            ...
            ex.copyWith(sets: [
              for (int i = 0; i < ex.sets.length; i++)
                i == setIndex - 1 // setIndex is 1-based, list is 0-based
                    ? ex.sets[i].copyWith(
                        isDone: !ex.sets[i].isDone, weight: weight, reps: reps)
                    : ex.sets[i]
            ])
          else
            ex
      ...
      try {
        final updatedExercise =
            updatedRoutine.firstWhere((e) => e.id == exerciseId);
        final updatedSet = updatedExercise.sets[setIndex - 1];
        debugPrint(
            "ElenaApp Log: Set $setIndex del ejercicio $exerciseId actualizado a isDone: ${updatedSet.isDone}");
      } catch (e) {
        debugPrint(
            "ElenaApp Log: Error al imprimir debug del set actualizado: $e");
      }
    });
  }
