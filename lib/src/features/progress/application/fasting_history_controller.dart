...
class FastingHistoryController
    extends AutoDisposeNotifier<FastingHistoryState> {
  ...
  FastingHistoryState build() {
    ...
    Future.delayed(Duration.zero, _loadHistory);
    ...
    if (view == MetricType.week) {
      ...
      for (int i = 0; i < 7; i++) {
        ...
        points.add(ChartDataPoint(
            x: i, y: totalHours, label: weekDays[i], fullDate: currentDay));
      }
      ...
      for (int i = 1; i <= daysInMonth; i++) {
        ...
        points.add(ChartDataPoint(
            ...
            y: totalHours,
            label:
                (i % 5 == 0 || i == 1) ? '$i' : '', // Show label every 5 days
            fullDate: currentDay));
      }
      ...
      for (int i = 1; i <= 12; i++) {
        ...
        if (monthSessions.isNotEmpty) {
          ...
          for (var s in monthSessions) {
            totalDuration +=
                s.endTime!.difference(s.startTime).inMinutes / 60.0;
          }
...
class ChartDataPoint {
  ...
  ChartDataPoint(
      {required this.x, required this.y, required this.label, this.fullDate});
}
