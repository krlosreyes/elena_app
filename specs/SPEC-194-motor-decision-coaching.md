...
**Estado:** CLOSED — MVP completo (2026-06-09). inc1 (dominio + scorer puro), inc2 (application: mappers, generadores weak-pillar/circadiano, snapshot builder, providers), inc3 (UI: card "Tu siguiente paso" + ActionExplainerSheet en Home). **Telemetría completa:** `coaching_action_shown` + `followed` + `completed` (correlación recomendación→registro de pilar vía CoachingCompletionService) + `coaching_feedback_shown`, con param `phase` en shown/followed para segmentar por fase circadiana. **4 fuentes de candidatos:** weak-pillar, circadiano, adaptativo, orchestrator. **Feedback de cierre (RF-194-05) enganchado:** `coachingClosureFeedbackProvider` + `CycleCoachingFeedbackCard` en Home (gated por hasUnreadCycleClosure; `improved` del delta cycle-aware). **Anti-fatiga PERSISTIDO (RF-2.5):** `CoachingFatigueNotifier` (SharedPreferences) alimenta `shownTodayActionIds` + `ignoredStreakByActionId` al snapshot; el card registra `recordShown` (post-frame) y `CoachingCompletionService` resetea la racha al completar; rollover diario incrementa la racha de lo mostrado-no-completado.

**Deltas vs. diseño / deuda diferida (no bloqueante):** `CoachingSnapshot` quedó más magro que la tabla RF-01 (faltan `fastingStatus/Hours`, `eatingWindowState`, `imrTrend`, `cycleState`, `streak`, `protocol`, `bodyData` — el motor opera con fase + pilar débil + circadiano + goals + anti-fatiga); el pilar recomendado del feedback de cierre = acción activa actual (no persistido por-ciclo); ML = SPEC-195 (post-PMF). La adenda circadiana se trackea en su propio doc (IMPLEMENTED).
**Versión:** 0.1 (draft)
