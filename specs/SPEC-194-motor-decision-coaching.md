...
**Estado:** IN-PROGRESS — MVP implementado: inc1 (dominio + scorer puro), inc2 (application: mappers, generadores weak-pillar/circadiano, snapshot builder, providers), inc3 (UI: card "Tu siguiente paso" + ActionExplainerSheet en Home). **Telemetría completa:** `coaching_action_shown` + `followed` + `completed` (correlación recomendación→registro de pilar vía CoachingCompletionService) + `coaching_feedback_shown`. **4 fuentes de candidatos:** weak-pillar, circadiano, adaptativo, orchestrator. **Feedback de cierre (RF-194-05) enganchado:** `coachingClosureFeedbackProvider` + `CycleCoachingFeedbackCard` en Home (gated por hasUnreadCycleClosure; `improved` del delta cycle-aware). **Anti-fatiga PERSISTIDO (RF-2.5, 2026-06-09):** `CoachingFatigueNotifier` (SharedPreferences) alimenta `shownTodayActionIds` + `ignoredStreakByActionId` al snapshot; el card registra `recordShown` (post-frame) y `CoachingCompletionService` resetea la racha al completar; rollover diario incrementa la racha de lo mostrado-no-completado. Antes estos inputs estaban vacíos → el motor nunca suprimía acciones ignoradas. **Pendiente:** param `phase` en telemetría; ML = SPEC-195 (post-PMF).
**Versión:** 0.1 (draft)
